...
describe('POST /api/skills/grocery/categories', () => {
  ...
  it('rejects duplicate category name', async () => {
    ...
    await expect(handler({ _body: { name: 'Fruits' } } as never)).rejects.toMatchObject({
      statusCode: 400,
  ...
  it('rejects empty name', async () => {
    ...
    await expect(handler({ _body: { name: '' } } as never)).rejects.toMatchObject({
      statusCode: 400,
    });
  });
});
...
describe('PATCH /api/skills/grocery/categories/:id', () => {
  ...
  it('updates an owned category', async () => {
    ...
    mockPrisma.groceryCategory.update.mockResolvedValue({
      id: 'cat-1',
      name: 'Légumes',
      sortOrder: 0,
    });
