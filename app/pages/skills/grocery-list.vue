<!--
  pages/skills/grocery-list.vue — Main grocery list skill page.
  Three tabs:
    - Liste     : in-store flow (quick-add, items grouped by category, drag-drop, checks)
    - Catégories: manage user categories (CRUD + reorder)
    - Catalogue : manage personal product catalog (search + remove outdated entries)
-->
<script setup lang="ts">
import { onMounted, ref, watch } from 'vue';
import { ShoppingCart, WifiOff, GripVertical, ListChecks, Tags, Archive } from 'lucide-vue-next';
import draggable from 'vuedraggable';
import { useGroceryList } from '~/composables/use-grocery-list';
import type { GroceryItem, GroceryItemInput } from '~/types/grocery';

definePageMeta({
  middleware: 'auth',
});

const {
  loading,
  error,
  activeItems,
  checkedItems,
  activeCount,
  itemsByCategory,
  sortedCategories,
  products,
  pendingSync,
  isOnline,
  fetchItems,
  fetchCategories,
  fetchProducts,
  addItem,
  toggleItem,
  updateItem,
  removeItem,
  clearChecked,
  reorderItems,
  addCategory,
  renameCategory,
  removeCategory,
  reorderCategories,
  removeProduct,
} = useGroceryList();

type Tab = 'list' | 'categories' | 'catalog';
const activeTab = ref<Tab>('list');

// Flat-list drag-to-reorder state — synced from activeItems.
const flatDraggableItems = ref<GroceryItem[]>([]);
watch(activeItems, (items) => { flatDraggableItems.value = [...items]; }, { immediate: true });

function onFlatListDragEnd() {
  const updated = flatDraggableItems.value.map((item, idx) => ({ id: item.id, sortOrder: idx }));
  reorderItems(updated);
}

// Item edit modal state.
const editingItem = ref<GroceryItem | null>(null);
const editModalOpen = ref(false);

function openEditItem(item: GroceryItem) {
  editingItem.value = item;
  editModalOpen.value = true;
}

function handleSaveEdit(payload: {
  id: string;
  data: Partial<Pick<GroceryItem, 'name' | 'quantity' | 'unit' | 'categoryId'>>;
}) {
  updateItem(payload.id, payload.data);
  editModalOpen.value = false;
  editingItem.value = null;
}

function handleAdd(input: GroceryItemInput) {
  addItem(input);
}

// Lazy-load the catalog only when its tab is opened — it can be large
// and isn't needed for the in-store flow.
const productsLoaded = ref(false);
watch(activeTab, async (tab) => {
  if (tab === 'catalog' && !productsLoaded.value) {
    await fetchProducts();
    productsLoaded.value = true;
  }
});

onMounted(async () => {
  await Promise.all([fetchItems(), fetchCategories()]);
});
</script>

<template>
  <div class="mx-auto max-w-2xl space-y-4 p-4 sm:p-6">
    <!-- Header -->
    <div class="flex items-center justify-between">
      <div class="flex items-center gap-3">
        <div class="rounded-lg bg-primary/10 p-2">
          <ShoppingCart class="h-5 w-5 text-primary" />
        </div>
        <div>
          <h1 class="text-2xl font-bold" data-testid="grocery-title">Liste de courses</h1>
          <p v-if="activeCount > 0 && activeTab === 'list'" class="text-sm text-percy-text-muted">
            {{ activeCount }} article{{ activeCount > 1 ? 's' : '' }} à acheter
          </p>
        </div>
      </div>

      <!-- Sync status -->
      <div
        v-if="!isOnline || pendingSync"
        class="flex items-center gap-1.5 text-xs text-percy-warning"
        data-testid="grocery-sync-status"
      >
        <WifiOff v-if="!isOnline" class="h-3.5 w-3.5" />
        <span v-if="!isOnline">Hors ligne</span>
        <span v-else-if="pendingSync">Synchronisation...</span>
      </div>
    </div>

    <!-- Tabs -->
    <div class="flex gap-1 border-b border-percy-border">
      <button
        class="flex items-center gap-2 border-b-2 px-4 py-2 text-sm"
        :class="
          activeTab === 'list'
            ? 'border-percy-primary text-percy-primary'
            : 'border-transparent text-percy-text-secondary hover:text-percy-text-primary'
        "
        data-testid="grocery-tab-list"
        @click="activeTab = 'list'"
      >
        <ListChecks class="h-4 w-4" />
        Liste
      </button>
      <button
        class="flex items-center gap-2 border-b-2 px-4 py-2 text-sm"
        :class="
          activeTab === 'categories'
            ? 'border-percy-primary text-percy-primary'
            : 'border-transparent text-percy-text-secondary hover:text-percy-text-primary'
        "
        data-testid="grocery-tab-categories"
        @click="activeTab = 'categories'"
      >
        <Tags class="h-4 w-4" />
        Catégories
      </button>
      <button
        class="flex items-center gap-2 border-b-2 px-4 py-2 text-sm"
        :class="
          activeTab === 'catalog'
            ? 'border-percy-primary text-percy-primary'
            : 'border-transparent text-percy-text-secondary hover:text-percy-text-primary'
        "
        data-testid="grocery-tab-catalog"
        @click="activeTab = 'catalog'"
      >
        <Archive class="h-4 w-4" />
        Catalogue
      </button>
    </div>

    <!-- Tab: Liste (in-store flow) -->
    <section v-if="activeTab === 'list'" class="space-y-4">
      <GroceryAddInput
        data-testid="grocery-add-section"
        :categories="sortedCategories"
        @add="handleAdd"
      />

      <div v-if="loading" class="space-y-3">
        <Skeleton class="h-10 w-full" />
        <Skeleton class="h-10 w-full" />
        <Skeleton class="h-10 w-3/4" />
      </div>

      <div
        v-else-if="error"
        class="rounded-lg border border-percy-danger/30 bg-percy-danger-light p-4 text-center text-sm text-percy-danger"
        data-testid="grocery-error"
      >
        {{ error }}
      </div>

      <div
        v-else-if="activeItems.length === 0 && checkedItems.length === 0"
        class="py-12 text-center"
        data-testid="grocery-empty"
      >
        <ShoppingCart class="mx-auto h-12 w-12 text-percy-text-muted" />
        <p class="mt-3 text-sm text-percy-text-muted">Votre liste est vide</p>
        <p class="text-xs text-percy-text-muted">Ajoutez un produit avec le champ ci-dessus</p>
      </div>

      <div v-else class="space-y-4">
        <template
          v-if="
            itemsByCategory.length > 1 ||
            (itemsByCategory.length === 1 && itemsByCategory[0]?.category)
          "
        >
          <GroceryCategoryGroup
            v-for="group in itemsByCategory"
            :key="group.category?.id ?? 'uncategorized'"
            :group="group"
            @toggle="toggleItem"
            @remove="removeItem"
            @reorder="reorderItems"
            @edit="openEditItem"
          />
        </template>

        <template v-else>
          <draggable
            v-model="flatDraggableItems"
            item-key="id"
            handle=".drag-handle"
            :animation="150"
            ghost-class="opacity-30"
            @end="onFlatListDragEnd"
          >
            <template #item="{ element: item }">
              <div class="group flex items-center gap-1">
                <button
                  class="drag-handle shrink-0 cursor-grab p-0.5 text-percy-text-muted opacity-0 transition-opacity group-hover:opacity-100 active:cursor-grabbing"
                  aria-label="Réordonner"
                  @click.stop
                >
                  <GripVertical class="h-3.5 w-3.5" />
                </button>
                <GroceryItemRow
                  :item="item"
                  class="flex-1"
                  @toggle="toggleItem($event)"
                  @remove="removeItem($event)"
                  @edit="openEditItem(item)"
                />
              </div>
            </template>
          </draggable>
        </template>

        <GroceryCheckedSection
          :items="checkedItems"
          @toggle="toggleItem"
          @remove="removeItem"
          @clear-all="clearChecked"
        />
      </div>
    </section>

    <!-- Tab: Catégories -->
    <section v-else-if="activeTab === 'categories'">
      <GroceryCategoriesTab
        :categories="sortedCategories"
        @add="addCategory"
        @rename="(payload) => renameCategory(payload.id, payload.name)"
        @remove="removeCategory"
        @reorder="reorderCategories"
      />
    </section>

    <!-- Tab: Catalogue -->
    <section v-else-if="activeTab === 'catalog'">
      <GroceryCatalogTab :products="products" @remove="removeProduct" />
    </section>

    <!-- Item edit modal (shared by all tabs) -->
    <GroceryItemEditModal
      :open="editModalOpen"
      :item="editingItem"
      :categories="sortedCategories"
      @close="editModalOpen = false"
      @save="handleSaveEdit"
    />
  </div>
</template>
